...
<template>
	<view class="components-theme">
		<wx-login v-if="!loginFlag" @loginSuccess="loginSuccess"></wx-login>
		<!-- 弹窗确认 -->
		<view>
			...
			<view class='UCenter-bg' :style="'background-image: url(' + pic[topBackGroupImageIndex].link + ');margin-top:' + CustomBar + 'px;'">
				...
				<block>
					<view class='text-center' @click="getUserInfo">
			...
			<block>
				<view class='padding flex text-center text-grey bg-white shadow-warp-my'>
					...
					<view class='flex flex-sub flex-direction solid-right'>
						<view class="text-xxl text-blue">3580 分钟</view>
						<view class="margin-top-sm">
							<text class='cuIcon-share'></text> 运动时间</view>
					</view>
					<view class='flex flex-sub flex-direction'>
						<view class="text-xxl text-red">95</view>
						<view class="margin-top-sm">
							<text class='cuIcon-like'></text> 健康评分</view>
					</view>
				</view>
				...
				<view class="cu-list menu card-menu margin-top-lg shadow-shop bg-white text-black my-radius sm-border">
					...
					<view class="cu-item " @click="userInfoUpdate">
		...
		<u-popup v-model="userPop" mode="bottom">
			<view style="padding: 20px 20px;">
				<u-form :model="form" ref="uForm" label-width="90px">
					...
					<u-form-item label="用户邮箱">
						<u-input v-model="userInfo.email" ></u-input>
					</u-form-item>
				</u-form>
	...
	export default {
		...
		methods: {
			...
			doSave() {
				...
				request.httpRequest(opts, this.userInfo).then(res => {
					console.log(res);
