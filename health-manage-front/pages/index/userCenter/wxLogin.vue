<template>
	<view class="mainBox bg-white">
		<view class="text-xxl" style="height: 486rpx; position: relative;">
			...
			<view class="logoName text-white text-bold text-xl">个人健康管理系统</view>
		</view>
		
		<view class="padding text-center margin-top-xl">
			<view class="padding-xl radius shadow-warp bg-white margin-top">
				<view class="describe margin-top-sm">个人健康管理系统，呵护您的健康。</view>
				<button class="margin-top-lg bg-gradual-blue" type="primary" lang="zh_CN" @click="login()">微信登录</button>
				...
				loginCode.then(code => {
						...
						return new Promise((resolve, reject) => {
							userProFile.then(res => {				
								...
								let form = {
									loginId: res.userInfo.nickName
								}
								request.httpRequest(opts, form).then(res => {
									console.log(res);
